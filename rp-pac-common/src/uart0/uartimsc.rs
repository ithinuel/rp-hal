/// Register `UARTIMSC` reader
pub type R = crate::register_blocks::R<UARTIMSC_SPEC>;
/// Register `UARTIMSC` writer
pub type W = crate::register_blocks::W<UARTIMSC_SPEC>;
/// Field `RIMIM` reader - nUARTRI modem interrupt mask. A read returns the current mask for the UARTRIINTR interrupt. On a write of 1, the mask of the UARTRIINTR interrupt is set. A write of 0 clears the mask.
pub type RIMIM_R = crate::register_blocks::BitReader;
/// Field `RIMIM` writer - nUARTRI modem interrupt mask. A read returns the current mask for the UARTRIINTR interrupt. On a write of 1, the mask of the UARTRIINTR interrupt is set. A write of 0 clears the mask.
pub type RIMIM_W<'a, REG> = crate::register_blocks::BitWriter<'a, REG>;
/// Field `CTSMIM` reader - nUARTCTS modem interrupt mask. A read returns the current mask for the UARTCTSINTR interrupt. On a write of 1, the mask of the UARTCTSINTR interrupt is set. A write of 0 clears the mask.
pub type CTSMIM_R = crate::register_blocks::BitReader;
/// Field `CTSMIM` writer - nUARTCTS modem interrupt mask. A read returns the current mask for the UARTCTSINTR interrupt. On a write of 1, the mask of the UARTCTSINTR interrupt is set. A write of 0 clears the mask.
pub type CTSMIM_W<'a, REG> = crate::register_blocks::BitWriter<'a, REG>;
/// Field `DCDMIM` reader - nUARTDCD modem interrupt mask. A read returns the current mask for the UARTDCDINTR interrupt. On a write of 1, the mask of the UARTDCDINTR interrupt is set. A write of 0 clears the mask.
pub type DCDMIM_R = crate::register_blocks::BitReader;
/// Field `DCDMIM` writer - nUARTDCD modem interrupt mask. A read returns the current mask for the UARTDCDINTR interrupt. On a write of 1, the mask of the UARTDCDINTR interrupt is set. A write of 0 clears the mask.
pub type DCDMIM_W<'a, REG> = crate::register_blocks::BitWriter<'a, REG>;
/// Field `DSRMIM` reader - nUARTDSR modem interrupt mask. A read returns the current mask for the UARTDSRINTR interrupt. On a write of 1, the mask of the UARTDSRINTR interrupt is set. A write of 0 clears the mask.
pub type DSRMIM_R = crate::register_blocks::BitReader;
/// Field `DSRMIM` writer - nUARTDSR modem interrupt mask. A read returns the current mask for the UARTDSRINTR interrupt. On a write of 1, the mask of the UARTDSRINTR interrupt is set. A write of 0 clears the mask.
pub type DSRMIM_W<'a, REG> = crate::register_blocks::BitWriter<'a, REG>;
/// Field `RXIM` reader - Receive interrupt mask. A read returns the current mask for the UARTRXINTR interrupt. On a write of 1, the mask of the UARTRXINTR interrupt is set. A write of 0 clears the mask.
pub type RXIM_R = crate::register_blocks::BitReader;
/// Field `RXIM` writer - Receive interrupt mask. A read returns the current mask for the UARTRXINTR interrupt. On a write of 1, the mask of the UARTRXINTR interrupt is set. A write of 0 clears the mask.
pub type RXIM_W<'a, REG> = crate::register_blocks::BitWriter<'a, REG>;
/// Field `TXIM` reader - Transmit interrupt mask. A read returns the current mask for the UARTTXINTR interrupt. On a write of 1, the mask of the UARTTXINTR interrupt is set. A write of 0 clears the mask.
pub type TXIM_R = crate::register_blocks::BitReader;
/// Field `TXIM` writer - Transmit interrupt mask. A read returns the current mask for the UARTTXINTR interrupt. On a write of 1, the mask of the UARTTXINTR interrupt is set. A write of 0 clears the mask.
pub type TXIM_W<'a, REG> = crate::register_blocks::BitWriter<'a, REG>;
/// Field `RTIM` reader - Receive timeout interrupt mask. A read returns the current mask for the UARTRTINTR interrupt. On a write of 1, the mask of the UARTRTINTR interrupt is set. A write of 0 clears the mask.
pub type RTIM_R = crate::register_blocks::BitReader;
/// Field `RTIM` writer - Receive timeout interrupt mask. A read returns the current mask for the UARTRTINTR interrupt. On a write of 1, the mask of the UARTRTINTR interrupt is set. A write of 0 clears the mask.
pub type RTIM_W<'a, REG> = crate::register_blocks::BitWriter<'a, REG>;
/// Field `FEIM` reader - Framing error interrupt mask. A read returns the current mask for the UARTFEINTR interrupt. On a write of 1, the mask of the UARTFEINTR interrupt is set. A write of 0 clears the mask.
pub type FEIM_R = crate::register_blocks::BitReader;
/// Field `FEIM` writer - Framing error interrupt mask. A read returns the current mask for the UARTFEINTR interrupt. On a write of 1, the mask of the UARTFEINTR interrupt is set. A write of 0 clears the mask.
pub type FEIM_W<'a, REG> = crate::register_blocks::BitWriter<'a, REG>;
/// Field `PEIM` reader - Parity error interrupt mask. A read returns the current mask for the UARTPEINTR interrupt. On a write of 1, the mask of the UARTPEINTR interrupt is set. A write of 0 clears the mask.
pub type PEIM_R = crate::register_blocks::BitReader;
/// Field `PEIM` writer - Parity error interrupt mask. A read returns the current mask for the UARTPEINTR interrupt. On a write of 1, the mask of the UARTPEINTR interrupt is set. A write of 0 clears the mask.
pub type PEIM_W<'a, REG> = crate::register_blocks::BitWriter<'a, REG>;
/// Field `BEIM` reader - Break error interrupt mask. A read returns the current mask for the UARTBEINTR interrupt. On a write of 1, the mask of the UARTBEINTR interrupt is set. A write of 0 clears the mask.
pub type BEIM_R = crate::register_blocks::BitReader;
/// Field `BEIM` writer - Break error interrupt mask. A read returns the current mask for the UARTBEINTR interrupt. On a write of 1, the mask of the UARTBEINTR interrupt is set. A write of 0 clears the mask.
pub type BEIM_W<'a, REG> = crate::register_blocks::BitWriter<'a, REG>;
/// Field `OEIM` reader - Overrun error interrupt mask. A read returns the current mask for the UARTOEINTR interrupt. On a write of 1, the mask of the UARTOEINTR interrupt is set. A write of 0 clears the mask.
pub type OEIM_R = crate::register_blocks::BitReader;
/// Field `OEIM` writer - Overrun error interrupt mask. A read returns the current mask for the UARTOEINTR interrupt. On a write of 1, the mask of the UARTOEINTR interrupt is set. A write of 0 clears the mask.
pub type OEIM_W<'a, REG> = crate::register_blocks::BitWriter<'a, REG>;
impl R {
    /// Bit 0 - nUARTRI modem interrupt mask. A read returns the current mask for the UARTRIINTR interrupt. On a write of 1, the mask of the UARTRIINTR interrupt is set. A write of 0 clears the mask.
    #[inline(always)]
    pub fn rimim(&self) -> RIMIM_R {
        RIMIM_R::new((self.bits & 1) != 0)
    }
    /// Bit 1 - nUARTCTS modem interrupt mask. A read returns the current mask for the UARTCTSINTR interrupt. On a write of 1, the mask of the UARTCTSINTR interrupt is set. A write of 0 clears the mask.
    #[inline(always)]
    pub fn ctsmim(&self) -> CTSMIM_R {
        CTSMIM_R::new(((self.bits >> 1) & 1) != 0)
    }
    /// Bit 2 - nUARTDCD modem interrupt mask. A read returns the current mask for the UARTDCDINTR interrupt. On a write of 1, the mask of the UARTDCDINTR interrupt is set. A write of 0 clears the mask.
    #[inline(always)]
    pub fn dcdmim(&self) -> DCDMIM_R {
        DCDMIM_R::new(((self.bits >> 2) & 1) != 0)
    }
    /// Bit 3 - nUARTDSR modem interrupt mask. A read returns the current mask for the UARTDSRINTR interrupt. On a write of 1, the mask of the UARTDSRINTR interrupt is set. A write of 0 clears the mask.
    #[inline(always)]
    pub fn dsrmim(&self) -> DSRMIM_R {
        DSRMIM_R::new(((self.bits >> 3) & 1) != 0)
    }
    /// Bit 4 - Receive interrupt mask. A read returns the current mask for the UARTRXINTR interrupt. On a write of 1, the mask of the UARTRXINTR interrupt is set. A write of 0 clears the mask.
    #[inline(always)]
    pub fn rxim(&self) -> RXIM_R {
        RXIM_R::new(((self.bits >> 4) & 1) != 0)
    }
    /// Bit 5 - Transmit interrupt mask. A read returns the current mask for the UARTTXINTR interrupt. On a write of 1, the mask of the UARTTXINTR interrupt is set. A write of 0 clears the mask.
    #[inline(always)]
    pub fn txim(&self) -> TXIM_R {
        TXIM_R::new(((self.bits >> 5) & 1) != 0)
    }
    /// Bit 6 - Receive timeout interrupt mask. A read returns the current mask for the UARTRTINTR interrupt. On a write of 1, the mask of the UARTRTINTR interrupt is set. A write of 0 clears the mask.
    #[inline(always)]
    pub fn rtim(&self) -> RTIM_R {
        RTIM_R::new(((self.bits >> 6) & 1) != 0)
    }
    /// Bit 7 - Framing error interrupt mask. A read returns the current mask for the UARTFEINTR interrupt. On a write of 1, the mask of the UARTFEINTR interrupt is set. A write of 0 clears the mask.
    #[inline(always)]
    pub fn feim(&self) -> FEIM_R {
        FEIM_R::new(((self.bits >> 7) & 1) != 0)
    }
    /// Bit 8 - Parity error interrupt mask. A read returns the current mask for the UARTPEINTR interrupt. On a write of 1, the mask of the UARTPEINTR interrupt is set. A write of 0 clears the mask.
    #[inline(always)]
    pub fn peim(&self) -> PEIM_R {
        PEIM_R::new(((self.bits >> 8) & 1) != 0)
    }
    /// Bit 9 - Break error interrupt mask. A read returns the current mask for the UARTBEINTR interrupt. On a write of 1, the mask of the UARTBEINTR interrupt is set. A write of 0 clears the mask.
    #[inline(always)]
    pub fn beim(&self) -> BEIM_R {
        BEIM_R::new(((self.bits >> 9) & 1) != 0)
    }
    /// Bit 10 - Overrun error interrupt mask. A read returns the current mask for the UARTOEINTR interrupt. On a write of 1, the mask of the UARTOEINTR interrupt is set. A write of 0 clears the mask.
    #[inline(always)]
    pub fn oeim(&self) -> OEIM_R {
        OEIM_R::new(((self.bits >> 10) & 1) != 0)
    }
}
impl core::fmt::Debug for R {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        f.debug_struct("UARTIMSC")
            .field("oeim", &self.oeim())
            .field("beim", &self.beim())
            .field("peim", &self.peim())
            .field("feim", &self.feim())
            .field("rtim", &self.rtim())
            .field("txim", &self.txim())
            .field("rxim", &self.rxim())
            .field("dsrmim", &self.dsrmim())
            .field("dcdmim", &self.dcdmim())
            .field("ctsmim", &self.ctsmim())
            .field("rimim", &self.rimim())
            .finish()
    }
}
impl W {
    /// Bit 0 - nUARTRI modem interrupt mask. A read returns the current mask for the UARTRIINTR interrupt. On a write of 1, the mask of the UARTRIINTR interrupt is set. A write of 0 clears the mask.
    #[inline(always)]
    pub fn rimim(&mut self) -> RIMIM_W<UARTIMSC_SPEC> {
        RIMIM_W::new(self, 0)
    }
    /// Bit 1 - nUARTCTS modem interrupt mask. A read returns the current mask for the UARTCTSINTR interrupt. On a write of 1, the mask of the UARTCTSINTR interrupt is set. A write of 0 clears the mask.
    #[inline(always)]
    pub fn ctsmim(&mut self) -> CTSMIM_W<UARTIMSC_SPEC> {
        CTSMIM_W::new(self, 1)
    }
    /// Bit 2 - nUARTDCD modem interrupt mask. A read returns the current mask for the UARTDCDINTR interrupt. On a write of 1, the mask of the UARTDCDINTR interrupt is set. A write of 0 clears the mask.
    #[inline(always)]
    pub fn dcdmim(&mut self) -> DCDMIM_W<UARTIMSC_SPEC> {
        DCDMIM_W::new(self, 2)
    }
    /// Bit 3 - nUARTDSR modem interrupt mask. A read returns the current mask for the UARTDSRINTR interrupt. On a write of 1, the mask of the UARTDSRINTR interrupt is set. A write of 0 clears the mask.
    #[inline(always)]
    pub fn dsrmim(&mut self) -> DSRMIM_W<UARTIMSC_SPEC> {
        DSRMIM_W::new(self, 3)
    }
    /// Bit 4 - Receive interrupt mask. A read returns the current mask for the UARTRXINTR interrupt. On a write of 1, the mask of the UARTRXINTR interrupt is set. A write of 0 clears the mask.
    #[inline(always)]
    pub fn rxim(&mut self) -> RXIM_W<UARTIMSC_SPEC> {
        RXIM_W::new(self, 4)
    }
    /// Bit 5 - Transmit interrupt mask. A read returns the current mask for the UARTTXINTR interrupt. On a write of 1, the mask of the UARTTXINTR interrupt is set. A write of 0 clears the mask.
    #[inline(always)]
    pub fn txim(&mut self) -> TXIM_W<UARTIMSC_SPEC> {
        TXIM_W::new(self, 5)
    }
    /// Bit 6 - Receive timeout interrupt mask. A read returns the current mask for the UARTRTINTR interrupt. On a write of 1, the mask of the UARTRTINTR interrupt is set. A write of 0 clears the mask.
    #[inline(always)]
    pub fn rtim(&mut self) -> RTIM_W<UARTIMSC_SPEC> {
        RTIM_W::new(self, 6)
    }
    /// Bit 7 - Framing error interrupt mask. A read returns the current mask for the UARTFEINTR interrupt. On a write of 1, the mask of the UARTFEINTR interrupt is set. A write of 0 clears the mask.
    #[inline(always)]
    pub fn feim(&mut self) -> FEIM_W<UARTIMSC_SPEC> {
        FEIM_W::new(self, 7)
    }
    /// Bit 8 - Parity error interrupt mask. A read returns the current mask for the UARTPEINTR interrupt. On a write of 1, the mask of the UARTPEINTR interrupt is set. A write of 0 clears the mask.
    #[inline(always)]
    pub fn peim(&mut self) -> PEIM_W<UARTIMSC_SPEC> {
        PEIM_W::new(self, 8)
    }
    /// Bit 9 - Break error interrupt mask. A read returns the current mask for the UARTBEINTR interrupt. On a write of 1, the mask of the UARTBEINTR interrupt is set. A write of 0 clears the mask.
    #[inline(always)]
    pub fn beim(&mut self) -> BEIM_W<UARTIMSC_SPEC> {
        BEIM_W::new(self, 9)
    }
    /// Bit 10 - Overrun error interrupt mask. A read returns the current mask for the UARTOEINTR interrupt. On a write of 1, the mask of the UARTOEINTR interrupt is set. A write of 0 clears the mask.
    #[inline(always)]
    pub fn oeim(&mut self) -> OEIM_W<UARTIMSC_SPEC> {
        OEIM_W::new(self, 10)
    }
}
/// Interrupt Mask Set/Clear Register, UARTIMSC  /// /// You can [`read`](crate::register_blocks::Reg::read) this register and get [`uartimsc::R`](R). You can [`reset`](crate::register_blocks::Reg::reset), [`write`](crate::register_blocks::Reg::write), [`write_with_zero`](crate::register_blocks::Reg::write_with_zero) this register using [`uartimsc::W`](W). You can also [`modify`](crate::register_blocks::Reg::modify) this register. See [API](https://docs.rs/svd2rust/#read--modify--write-api).
pub struct UARTIMSC_SPEC;
impl crate::register_blocks::RegisterSpec for UARTIMSC_SPEC {
    type Ux = u32;
}
/// `read()` method returns [`uartimsc::R`](R) reader structure
impl crate::register_blocks::Readable for UARTIMSC_SPEC {}
/// `write(|w| ..)` method takes [`uartimsc::W`](W) writer structure
impl crate::register_blocks::Writable for UARTIMSC_SPEC {
    type Safety = crate::register_blocks::Unsafe;
}
/// `reset()` method sets UARTIMSC to value 0
impl crate::register_blocks::Resettable for UARTIMSC_SPEC {}
